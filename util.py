...
class GomokuUtil(object):
    ...
    def check_five_in_row(self, board_state):
        ...
        '''
        size = len(board_state)
        black_pattern = [self.color_dict[self.BLACK]
                         for _ in range(5)]  # [1,1,1,1,1]
        white_pattern = [self.color_dict[self.WHITE]
                         for _ in range(5)]  # [2,2,2,2,2]
        ...
        if (black_win and white_win):
            raise error.Error(
                'Both Black and White has 5-in-row, rules conflicts')
    ...
    def check_pattern_index(self, board_state, pattern):
        ...
        if (exist_patttern):
            ...
            # line: list[list[(x1, y1),...]], startlist: list[int]
            return lines, startlist
...
def make_beginner_policy(np_random):
    ...
    def defend_policy(curr_state):
        ...
        pattern_four_a = [
            0] + [gomoku_util.color_dict[opponent_color]] * 4  # [0,1,1,1,1]
        pattern_four_b = [
            gomoku_util.color_dict[opponent_color]] * 4 + [0]  # [1,1,1,1,0]
        pattern_three_a = [
            0] + [gomoku_util.color_dict[opponent_color]] * 3 + [0]  # [0,1,1,1,0]
        pattern_three_b = [gomoku_util.color_dict[opponent_color]] * 2 + \
            [0] + [gomoku_util.color_dict[opponent_color]] * 1  # [1,1,0,1]
        pattern_three_c = [gomoku_util.color_dict[opponent_color]] * 1 + \
            [0] + [gomoku_util.color_dict[opponent_color]] * 2  # [1,0,1,1]
        patterns = [pattern_four_a, pattern_four_b,
                    pattern_three_a, pattern_three_b, pattern_three_c]
    ...
    def fill_box(board, coord):
        ...
        if (coord[0] >= 0):  # last move coord should be within the board
            box = [(i, j) for i in range(coord[0]-1, coord[0] + 2)
                   for j in range(coord[1]-1, coord[1] + 2)]  # 3x3 box
            legal_moves = []
            for c in box:
            ...
            if (len(legal_moves) == 0):
                # all the box is full
                next_move = all_legal_moves[np_random.choice(
                    len(all_legal_moves))]
    ...
    def connect_line(board, pattern):
        ...
        lines, starts = gomoku_util.check_pattern_index(
            board.board_state, pattern)
    ...
    def strike_policy(curr_state, prev_state, prev_action):
        ...
        pattern_four_a = [
            0] + [gomoku_util.color_dict[player_color]] * 4  # [0,1,1,1,1]
        pattern_four_b = [
            gomoku_util.color_dict[player_color]] * 4 + [0]  # [1,1,1,1,0]
        pattern_three_a = [
            0] + [gomoku_util.color_dict[player_color]] * 3 + [0]  # [0,1,1,1,0]
        pattern_three_b = [gomoku_util.color_dict[player_color]] * 2 + \
            [0] + [gomoku_util.color_dict[player_color]] * 1  # [1,1,0,1]
        pattern_three_c = [gomoku_util.color_dict[player_color]] * 1 + \
            [0] + [gomoku_util.color_dict[player_color]] * 2  # [1,0,1,1]
        pattern_two = [0] + [gomoku_util.color_dict[player_color]
                             ] * 2 + [0]  # [0,1,1,0]
        patterns = [pattern_four_a, pattern_four_b, pattern_three_a,
                    pattern_three_b, pattern_three_c, pattern_two]
...
def make_expert_policy():
    ...
    def expert_policy(curr_state, prev_state, prev_action):
        '''To Do'''
        return next(a)
